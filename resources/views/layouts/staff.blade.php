    ...
    <div class="staff-shell">
        <aside class="staff-sidebar" aria-label="Staff navigation / تنقل الموظفين">
            ...
            <nav class="staff-nav" aria-label="Navigation">
                <a class="{{ request()->routeIs('staff.dashboard') ? 'is-active' : '' }}" href="{{ route('staff.dashboard') }}">
                    <span class="nav-icon" aria-hidden="true">⌂</span>
                    <span>Tableau de bord<small>لوحة التحكم</small></span>
                </a>
                <a class="{{ request()->routeIs('staff.orders') ? 'is-active' : '' }}" href="{{ route('staff.orders') }}">
                    <span class="nav-icon" aria-hidden="true">▣</span>
                    <span>File des commandes<small>قائمة الطلبات</small></span>
                </a>
                <a class="{{ request()->routeIs('staff.service-requests') ? 'is-active' : '' }}" href="{{ route('staff.service-requests') }}">
                    <span class="nav-icon" aria-hidden="true">♢</span>
                    <span>Demandes de service<small>طلبات الخدمة</small></span>
                </a>
                <a class="{{ request()->routeIs('staff.tables') ? 'is-active' : '' }}" href="{{ route('staff.tables') }}">
                    <span class="nav-icon" aria-hidden="true">▤</span>
                    <span>Détails de la table<small>تفاصيل الطاولة</small></span>
                </a>
                <a class="{{ request()->routeIs('staff.payments') ? 'is-active' : '' }}" href="{{ route('staff.payments') }}">
                    <span class="nav-icon" aria-hidden="true">▥</span>
            ...
            <div class="staff-sidebar__footer">
                <span class="staff-open-state"><i></i> Ouvert<small>مفتوح</small></span>
                <span class="staff-clock">{{ now()->format('H:i') }}<small>{{ now()->format('D d M') }}</small></span>
                <span class="connection-indicator" data-connection-indicator>Checking connection / جار التحقق</span>
                <span class="staff-role">{{ ucfirst($role ?? 'staff') }}<small>الموظفون</small></span>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="button button--quiet" type="submit">Logout / خروج</button>
                </form>
            </div>
        </aside>
            ...
            <header class="staff-topbar">
                ...
                <div class="staff-topbar__actions">
                    ...
                    <span class="staff-user" aria-label="Current user">
                        <b>◉</b>
                        <span>{{ ucfirst($role ?? 'staff') }}<small>Serveur</small></span>
                    </span>
